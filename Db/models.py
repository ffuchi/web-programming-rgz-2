...
class cinema_sessions(db.Model):
    session_id = db.Column(db.INT, primary_key=True)
    movie = db.Column(db.VARCHAR(255), nullable=False)
    room_number = db.Column(db.INT, nullable=False)
    start_time = db.Column(db.DATETIME, nullable=False)


class seats(db.Model):
    id = db.Column(db.INT, primary_key=True)
    session_id = db.Column(db.INT, db.ForeignKey('cinema_sessions.session_id'), nullable=False)
    seat_number = db.Column(db.INT, nullable=False)
    reserved_by = db.Column(db.INT, db.ForeignKey('users.id'))
